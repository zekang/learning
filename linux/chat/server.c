...
int server_init(USER ***users/*out*/,int count,const char *fifo_name,int *pfd/*out*/) /*{{{*/
{
		int ret = 0;
		int fd = 0;  
		USER **tmp = NULL;
		if(count < 1 || users == NULL || pfd==NULL){
				chat_error(CHAT_INFO,"count <1 or users is null or pfd is null \n");
				return -1;
		}
		if(fifo_name == NULL){
...
int main(int argc,char *argv[]) /*{{{*/
{
		...
		int fd = -1,ret;
		if(argc > 1){
				max_user = atoi(argv[1]);
		}
		if(max_user<2){
				chat_error(CHAT_FATAL,"params %s is invalid \n",argv[1]);
		}
		ret = server_init(&users,max_user,NULL,&fd);
		if(ret != 0){
				goto END;
		}
		server_run(users,max_user,fd);	
END:
		server_destroy(users,max_user,NULL,fd);
		return 0;
}/*}}}*/
